...
{
  std :: string fileNameWithNoExtension = "Queuesize";
  std :: string graphicsFileName  = fileNameWithNoExtension + ".png";
  std :: string plotFileName  = fileNameWithNoExtension + ".plt";
  std :: string plotTitle  = "Graph-2";
  std :: string dataTitle  = "Packetdrop rate varied over Queuesize";
  ...
  plot.SetLegend("Queuesize(in packets)", "Packets dropped");
  // Set the range for the x axis.
  plot.AppendExtra("set xrange [110:0]");
 ...
 for(int Qsize=100;Qsize>=0;Qsize-=15)
{
  ...
  pointToPointRouter.SetQueue ("ns3::DropTailQueue","MaxSize", StringValue (std::to_string(Qsize)+"p"));
...
//
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    ...
    dataset.Add(Qsize, Sumx);
    std :: cout << " Packetsdropped: " << Sumx << std :: endl;
